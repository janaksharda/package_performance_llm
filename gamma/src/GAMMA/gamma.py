...
class GAMMA(object):
    def __init__(self,dimension, map_cstr=None, num_pe=64, pe_limit=1024, package1 = None, fitness="latency", constraints=dict(), par_RS=False, 
                  l1_size=512, l2_size=108000, NocBW=81920000, offchipBW=81920000, slevel_min=2, slevel_max=2, fixedCluster=0, log_level=2,
                  constraint_class=None, external_mem_cstr=None, use_factor=False, uni_base=True, layer_no=0, layer_multiplier=1,vector_width=16,
                  store_output=False, store_input=False, store_weight=False, store_output_l2=False, store_input_l2=False, store_weight_l2=False):
        ...
        self.store_output_l2 = store_output_l2
        self.store_input_l2 = store_input_l2
        self.store_weight_l2 = store_weight_l2
        self.best_pe_util = 0
    ...
    def adjust_fitness(self, fitness):
        ...
        rank = rank1 + self.best_pe_util*rank2
        fitness[:,0] = rank
    ...
    def evaluate(self, pool, population, cur_gen=-1):
        ...
        for i in range(len(population)):
            reward, activity_count, df_list, pe_util = reward_activ_list[i]
        ...
        if judging_best_reward < gen_best:
            ...
            self.best_df = copy.deepcopy(best_df)
            self.best_pe_util = copy.deepcopy(best_pe_util)
    ...
    def run(self, dimension, stage_idx=0, prev_stage_value=0, num_population=100, num_generations=100, elite_ratio=0.4, parents_ratio=0.2, ratio_decay=1, 
              num_finetune=1, best_sol_1st=None, init_pop=None, bias=None, uni_base=True, use_factor=False, use_pleteau=False, L1_bias_template=None, df_list=None):
        ...
        for g in range(num_generations):
            ...
            self.crossover_tile(self.parents, population, alpha=(0.1)**self.best_pe_util)
            if self.constraint_class == "1000":
                ...
                self.swap_order(population, alpha=(0.1)**self.best_pe_util)
                self.mutate_tile(population, num_mu_loc=3, range_alpha=(0.1)**self.best_pe_util, alpha=(0.1)**self.best_pe_util, is_finetune=False)
                self.mutate_pe(population, alpha=1 if g==0 else 0.5) if self.num_pe<1 else None
                self.mutate_par(population, alpha=(0.1)**self.best_pe_util)


            if self.map_cstr is None:
                self.born_cluster(population, alpha=0.8)
                self.kill_cluster(population, alpha=0.1)
        ...
        self.population = population[:self.num_population]
        pool.close()
    ...
    def thread_fun(self, individual):
        reward, activity_count, df_list, pe_util, _, _ = self.oberserve_maestro(individual)
        return [reward, activity_count, df_list, pe_util]

    def get_indiv_info(self, individual, num_pe=None, l1_size=None, l2_size=None, NocBW=None, print_info=False):
        _,_,_,_, chiplet_str, package_str = self.oberserve_maestro(individual,num_pe=num_pe, l1_size=l1_size, l2_size=l2_size, NocBW=NocBW, print_info=print_info)
        return self.observation, chiplet_str, package_str
    ...
    def oberserve_maestro(self, indv, num_pe=None, l1_size=None, l2_size=None, NocBW=None, offchipBW=None, print_info=False):
        ...
        command = [self._executable,
                   ...
                   "--store_output={}".format(self.store_output if self.store_output is not None else False),
                   "--store_input={}".format(self.store_input if self.store_input is not None else False),
                   "--store_weight={}".format(self.store_weight if self.store_weight is not None else False),
                   "--store_output_l2={}".format(self.store_output_l2 if self.store_output_l2 is not None else False),
                   "--store_input_l2={}".format(self.store_input_l2 if self.store_input_l2 is not None else False),
                   "--store_weight_l2={}".format(self.store_weight_l2 if self.store_weight_l2 is not None else False),
                   "--full_buffer=false", "--noc_bw_cstr={}".format(self.NocBW if not NocBW else NocBW),
                   "--noc_hops=1", "--noc_hop_latency=1", "--do_dse=false", 
                   ...
                   "--num_simd_lanes={}".format(self.vector_width), "--l1_size_cstr={}".format(self.l1_size if not l1_size else l1_size),
        ...
        try:
            ...
            df[" Offchip BW Req (Elements/cycle)"] = min((np.array(df[" Offchip BW Req (Elements/cycle)"]), self.offchipBW))
            df[" NoC BW Req (Elements/cycle)"] = min((np.array(df[" NoC BW Req (Elements/cycle)"]), self.NocBW))

            layer_name = df[" Layer Number"]
            numpe = np.array(df[" NumPEs"]).reshape(-1, 1)
            ...
            if mac <= 0:
                ...
                pe_utilization = mac / (runtime * numpe * self.vector_width)
            os.remove("./{}.csv".format(m_file))  if os.path.exists("./{}.csv".format(m_file)) else None
            ...
            layer_trace = arrange_pandas_df(df)
            df_temp = copy.deepcopy(self.df_list)
            df_temp[self.layer_no] = layer_trace
            self.package1.package.assign_layer_performance(df_temp)
            chiplet_str, package_str = self.package1.package.system_level_analysis(print_info=print_info)

            self.observation = [np.mean(x) for x in [runtime, self.package1.package.latency, self.package1.package.total_energy, self.package1.package.total_area, \
                                    pe_utilization, self.package1.package.noc_bw, self.package1.package.dram_bw, self.package1.package.l2_size]]

            def catch_exception():
                if l1_size>self.l1_size or l2_size>self.l2_size or any(runtime_series<1) or any(l1_size_series<1) or any(l2_size_series<1) or mac<0 or self.package1.package.total_area/u.mm ** 2 < 0:
                    return True
                else:
                    return False
    ...
    def judge(self):
        ...
        layer_latency, runtime, energy, area, pe_util, noc_bw, dram_bw, l2_size = self.observation
        def get_objective(objective):
            values = []

            ## merge objective and self.constraints into one list and remove duplicates, keep order such that objective is first
            ### self.constraints is a dictionary
            objective_new = list(objective)
            for term in self.constraints.keys():
                if term not in objective_new:
                    objective_new.append(term)
           
            for term in objective_new:
                # if term[:1] == "n":
                #     term = term[1:]
                if term == "energy":
                    reward = -energy/(u.mJ)# - l2_size/(u.kB)
                elif term == "thrpt_ave":
                    reward = throughput
                elif term == "EDP":
                    ...
                    reward = -runtime
                elif term == "layer_latency":
                    reward = -layer_latency
                elif term == "area":
                    reward = -area
                elif term == "l1_size":
                    reward = - l1_size / u.MB
                ...
                if term in self.constraints.keys():
                    if reward < -self.constraints[term]:
                        # print("Constraint violated: ", term, reward, self.constraints[term])
    ...
    def init_arguement(self, dimension=None, stage_idx=0, prev_stage_value=0, num_population=100, num_generations=100,
                       elite_ratio=0.4,
                       parents_ratio=0.2, ratio_decay=1, num_finetune=1, best_sol_1st=None, init_pop=None, uni_base=False, use_factor=False, use_pleteau=False,L1_bias_template=None):
